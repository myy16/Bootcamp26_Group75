export type StoreName = "Watsons" | "Gratis" | "Mion" | "Rossmann";
...
export function getStoreTotals(items: Product[]): Record<StoreName, number> {
  const totals: Record<StoreName, number> = {
    Watsons: 0,
    Gratis: 0,
    Rossmann: 0,
    Mion: 0,
...
export function getCheapestTotal(items: Product[]): {
  total: number;
  breakdown: { product: Product; store: StorePrice }[];
} {
  const breakdown = items.map((p) => ({
    product: p,
    store: getCheapestStore(p.stores),
  }));
...
export const ROUTINE_PRODUCTS: Product[] = [
  {
    id: "demo-p1",
    image:
      "https://images.unsplash.com/photo-1746227638992-50b1e1e0d96b?auto=format&fit=crop&w=600&q=80",
    brand: "CeraVe",
  ...
  },
  {
    id: "demo-p2",
    image:
      "https://images.unsplash.com/photo-1748543668676-ea8241cb3886?auto=format&fit=crop&w=600&q=80",
  ...
  },
  {
    id: "demo-p3",
    image:
      "https://images.unsplash.com/photo-1748543668751-902d6461890d?auto=format&fit=crop&w=600&q=80",
    brand: "Bioderma",
